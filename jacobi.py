...
def jacobi_method(A, b, x0=None, tol=1e-6, max_iter=100, print_table=True):
    n = len(b)
    if x0 is None:
        x = np.zeros(n)
    else:
    ...
    diag_dominant = all(2 * abs(D[i]) >= sum(abs(A[i, :])) for i in range(n))
    ...
    for k in range(1, max_iter + 1):
        ...
        if print_table:
            print(f"{k:<6} {np.array2string(x_new, precision=6, suppress_small=True):<30} {error:<15.8f}")
        
        if error < tol:
            if print_table:
                print(f"\nConverged in {k} iterations.")
                print(f"Solution: {np.array2string(x_new, precision=8, suppress_small=True)}")
                print(f"Residual norm: {np.linalg.norm(A @ x_new - b):.2e}")
            return x_new, k, True, history
...
if __name__ == "__main__":
    print("Jacobi Method - Linear System Solver")
    print("=" * 50)
    ...
    print("\n" + "="*60)
    sol_j, it_j, conv_j, _ = jacobi_method(A, b, tol=tol, max_iter=max_iter, print_table=print_table)
    
    exact = np.linalg.solve(A, b)
    print("\n" + "="*60)
    print("Summary:")
    print(f"Jacobi solution: {np.array2string(sol_j, precision=8, suppress_small=True)}  (iterations: {it_j}, converged: {conv_j})")
